...
def build_dataset(file_pattern,
  ...
  logging.info("Building input pipeline from %d files matching patterns: %s",
                  len(filenames), file_patterns)

  if include_labels:
    if not input_config.labels_are_columns:
  ...
  def _example_parser(serialized_example):
    ...
    if include_labels:
      if input_config.labels_are_columns:
    ...
    if include_identifiers:
      identifier_fields = ["tic_id"]
      for f in identifier_fields:
        if f in data_fields:
          raise ValueError("Features include illegal field {}".format(f))
        data_fields[f] = tf.io.FixedLenFeature([], tf.int64)
    ...
    # Reorganize outputs.
    output = {"features": {}, "identifiers": {}}
    
    if include_labels and input_config.labels_are_columns:
      labels = tf.stack(
          [parsed_features[name] for name in input_config.label_columns])
      output["labels"] = tf.cast(tf.minimum(labels, 1), tf.float32)

      labels_f = tf.cast(labels, tf.float32)
      output["weights"] = tf.reduce_max(labels_f) / tf.maximum(tf.reduce_sum(labels_f), 1.0)
      if input_config.example_weighting:
        if labels[input_config.primary_class] < 1:
          output["weights"] /= 2.0
    
    for feature_name, value in parsed_features.items():
      if feature_name == input_config.label_feature:
        if include_labels and not input_config.labels_are_columns:
            ...
            if one_hot_labels:
              output["labels"] = tf.one_hot(label_id, len(input_config.label_map))
            else:
              output["labels"] = label_id
      elif feature_name in input_config.label_columns:
        continue
      elif include_identifiers and feature_name in identifier_fields:
        if include_identifiers:
          output["identifiers"][feature_name] = value
        ...
        assert feature_name not in output["features"]
        output["features"][feature_name] = value
      else:
        assert feature_name not in output["features"]
        use_value = True
        if input_config.features[feature_name].has_nans:
            if tf.math.is_nan(value):
                use_value = False
                mask = tf.zeros_like(value)
            else:
                mask = tf.ones_like(value)
            output["features"][feature_name + "_present"] = mask
        if use_value:
            mean = input_config.features[feature_name]['mean']
            std = input_config.features[feature_name]['std']
  ...
  def split_features_labels(records):
    if include_labels:
      if "weights" in records:
        return records["features"], records["labels"], records["weights"]
      else:
        return records["features"], records["labels"]
    elif include_identifiers:
      return records["features"], records["identifiers"]
    else:
