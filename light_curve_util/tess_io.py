...
def tess_filenames(tic, base_dir='/pdo/qlp-data/', sector=1, cam=1, ccd=1, check_existence=True):
...
def read_tess_light_curve(filename, flux_key='RawMagnitude'):
    ...
    cadence = np.array(f['LightCurve']['Cadence'])
    if 'QFLAG' in f['LightCurve'].keys():
        ...
        time = time[valid_indices]
        mag = mag[valid_indices]
        cadence = cadence[valid_indicies]


    return time, mag, cadence
