...
def new_binning(time, flux, period, num_bins):
  ...
  for i,b in enumerate(bins_center):
    ...
    in_bin = t_c[np.where(abs(t_c) <= hbw + hc)]
    f_x = flux[np.where(abs(t_c) <= hbw + hc)]
    ...
    mean, mean_stddev, mask = robust_mean.robust_mean(f_x, 3)
    ...
    weight = [get_overlap(-hbw, hbw, in_bin[j]-hc, in_bin[j]+hc)/bin_width for j in range(len(in_bin))]
